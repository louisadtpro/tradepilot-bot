...
async def rapport_hebdo():
    ...
    embed = discord.Embed(
        ...
        description=(
            f"Semaine du **{lundi.strftime('%d/%m')}** au **{dimanche.strftime('%d/%m/%Y')}**\n\n"
            "Voici le recap des performances de la semaine :"
        ),
        color=0xF5B830
    )
    embed.add_field(name="📡 Signaux envoyes", value="A renseigner", inline=True)
    embed.add_field(name="✅ Gagnants", value="A renseigner", inline=True)
    embed.add_field(name="❌ Perdants", value="A renseigner", inline=True)
    embed.add_field(name="📈 Winrate", value="A renseigner", inline=True)
    embed.add_field(name="💰 Resultat", value="A renseigner", inline=True)
    embed.add_field(name="⭐ Meilleur trade", value="A renseigner", inline=True)
    embed.set_footer(text="TradePilot — Rapport automatique du lundi 9h00")
    embed.timestamp = datetime.now()

    await channel.send(embed=embed)
    print(f"✅ Rapport hebdo envoye")
