...
WEBHOOK_RESULTATS = "https://discord.com/api/webhooks/1519788846844346419/kfuhsYBsdsORjyeiSdH7oqwp-qWaQeMYz30j8TQYOmqBkuFbruX3D7NW-ilVa5lw3sTw"
...
class TradePilotBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        self.add_view(ProfilView())
        check_relance.start()
        rapport_hebdo.start()
        stats_live_update.start()
        print("✅ Views enregistrées et tasks démarrées")
...
@app.route("/parrainage", methods=["POST"])
def parrainage():
    """Appelee par Make.com quand un achat contient un code parrainage"""
    data = request.get_json()
    parrain_discord_id = data.get("parrain_id")    # ID Discord du parrain
    filleul_name = data.get("filleul_name", "quelqu'un")  # Nom du nouveau client
    filleul_email = data.get("filleul_email", "")

    if not parrain_discord_id:
        return jsonify({"status": "error", "message": "parrain_id manquant"}), 400

    asyncio.run_coroutine_threadsafe(
        send_parrainage_dm(int(parrain_discord_id), filleul_name, filleul_email), bot.loop
    )
    return jsonify({"status": "ok"})


async def send_parrainage_dm(parrain_id, filleul_name, filleul_email):
    """Envoie un DM au parrain pour lui annoncer son mois gratuit"""
    try:
        user = await bot.fetch_user(parrain_id)

        embed = discord.Embed(
            title="🎉 Tu as parrainé quelqu'un !",
            description=(
                f"**{filleul_name}** vient d'acheter une offre TradePilot "
                f"avec ton code de parrainage !\n\n"
                "**Ta recompense : 1 mois Signal VIP GRATUIT**\n"
                "Ton abonnement du mois prochain est offert.\n\n"
                "On te contacte dans les 24h pour activer ta recompense.\n\n"
                "Merci de faire confiance a TradePilot ! 🚀"
            ),
            color=0x00C97A
        )
        embed.set_footer(text="TradePilot — Programme Parrainage")
        await user.send(embed=embed)
        print(f"✅ DM parrainage envoye → {user.name}")

        # Notifier admin dans #ventes-dashboard
        guild = next((g for g in bot.guilds), None)
        if guild:
            channel = guild.get_channel(1519256169338638346)
            if channel:
                notif = discord.Embed(
                    title="🤝 Nouveau Parrainage !",
                    description=(
                        f"**{filleul_name}** ({filleul_email}) a achete via le parrainage de <@{parrain_id}>\n"
                        f"1 mois Signal VIP gratuit a accorder au parrain."
                    ),
                    color=0x00C97A
                )
        ...
        print(f"❌ Erreur parrainage DM : {e}")


@app.route("/annulation", methods=["POST"])
def annulation():
    """Webhook Stripe direct — customer.subscription.deleted"""
    ...
    # Traiter uniquement les annulations
    if event["type"] == "customer.subscription.deleted":
        ...
        try:
            ...
            if discord_id:
                asyncio.run_coroutine_threadsafe(
                    send_retention_dm(int(discord_id), email, name), bot.loop
                )
        except Exception as e:
            print(f"❌ Erreur recuperation client Stripe : {e}")

    return jsonify({"status": "ok"})


async def send_retention_dm(discord_id, email, name):
    """Envoie un DM de retention a un client qui annule"""
    try:
        user = await bot.fetch_user(discord_id)
        embed = discord.Embed(
            title="😢 On est tristes de te voir partir...",
            description=(
                f"Salut **{name}** !\n\n"
                "On a vu que tu as annule ton abonnement Signal VIP.\n\n"
                "Avant que tu partes, on aimerait te faire une offre speciale :\n\n"
                "**-20% sur ton prochain mois Signal VIP**\n"
                "~~59.99€~~ → **47.99€/mois**\n\n"
                "Utilise le code **STAY20** au moment de te reabonner.\n\n"
                "Tu as des questions ou un probleme ? Reponds a ce message "
                "ou ouvre un ticket dans le Discord — on est la pour toi.\n\n"
                "→ tradepilotautomatisation.netlify.app"
            ),
            color=0xF5B830
        )
        embed.set_footer(text="TradePilot — On espere te revoir bientot !")
        await user.send(embed=embed)

        # Retirer le role Signal VIP
        guild = next((g for g in bot.guilds), None)
        if guild:
            member = guild.get_member(discord_id)
            if member:
                role = guild.get_role(ROLES["signal"])
                if role and role in member.roles:
                    await member.remove_roles(role)
                    print(f"✅ Role Signal VIP retire — {member.display_name}")

        # Notifier dans ventes-dashboard
        channel = guild.get_channel(1519256169338638346) if guild else None
        if channel:
            notif = discord.Embed(
                title="❌ Annulation Signal VIP",
                description=f"**{name}** ({email}) a annule son abonnement. DM de retention envoye avec code STAY20.",
                color=0xFF4444
            )
            await channel.send(embed=notif)

        print(f"✅ DM retention envoye → {name}")
    except Exception as e:
        print(f"❌ Erreur retention DM : {e}")
...
# ── STATS MT5 (page résultats publique) ─────────────────
_mt5_stats = {}

@app.route("/stats", methods=["POST"])
def update_stats():
    data = request.json or {}
    _mt5_stats.update(data)
    _mt5_stats["updated_at"] = datetime.utcnow().strftime("%d/%m/%Y %H:%M UTC")
    return jsonify({"status": "ok"}), 200

@app.route("/stats", methods=["GET"])
def get_stats():
    return jsonify(_mt5_stats), 200
...
async def on_ready():
    ...
    # Attendre que le cache soit completement charge
    ...
    for attempt in range(10):
        ...
        if guild:
            break
        print(f"⏳ Retry {attempt+1}/10 — Guilds: {[(g.id, type(g.id)) for g in bot.guilds]}")
        await asyncio.sleep(2)

    if not guild:
        # Dernier recours — prendre le premier serveur disponible
        if bot.guilds:
            guild = bot.guilds[0]
            print(f"⚠️ Utilisation du premier serveur disponible: {guild.id}")
        else:
...
# ════════════════════════════════════════════════════════
# ── RAPPORT HEBDO ───────────────────────────────────────
RESULTATS_CHANNEL_ID = 1519635511101095966

@tasks.loop(hours=1)
async def rapport_hebdo():
    now = datetime.utcnow()  # Railway tourne en UTC
    if now.weekday() != 0 or now.hour != 7:  # 7h UTC = 9h France (UTC+2 été)
        return

    lundi = now - timedelta(days=7)
    dimanche = now - timedelta(days=1)

    payload = {
        "embeds": [{
            "title": "📊 RAPPORT HEBDOMADAIRE",
            "description": f"Semaine du **{lundi.strftime('%d/%m')}** au **{dimanche.strftime('%d/%m/%Y')}**\n\nVoici le recap des performances de la semaine :",
            "color": 0xF5B830,
            "fields": [
                {"name": "📡 Signaux envoyes", "value": "A renseigner", "inline": True},
                {"name": "✅ Gagnants", "value": "A renseigner", "inline": True},
                {"name": "❌ Perdants", "value": "A renseigner", "inline": True},
                {"name": "📈 Winrate", "value": "A renseigner", "inline": True},
                {"name": "💰 Resultat", "value": "A renseigner", "inline": True},
                {"name": "⭐ Meilleur trade", "value": "A renseigner", "inline": True},
            ],
            "footer": {"text": "TradePilot — Rapport automatique du lundi 9h00"}
        }]
    }

    try:
        http_requests.post(WEBHOOK_RESULTATS, json=payload)
        print("✅ Rapport hebdo envoye")
    except Exception as e:
        print(f"❌ Erreur rapport hebdo : {e}")


# ── COMMANDE PARRAINAGE ──────────────────────────────────
@bot.tree.command(name="moncode", description="Obtenir ton code de parrainage TradePilot")
async def moncode(interaction: discord.Interaction):
    """Donne au client son code de parrainage unique"""
    guild = interaction.guild
    member = interaction.user

    # Verifier que le membre a un role produit
    member_role_ids = [r.id for r in member.roles]
    has_product = any(rid in member_role_ids for rid in ROLES.values())

    if not has_product:
        await interaction.response.send_message(
            "❌ Tu dois avoir une offre active pour acceder au programme de parrainage.",
            ephemeral=True
        )
        return

    code = str(member.id)
    lien = f"https://tradepilotautomatisation.netlify.app?ref={code}"

    embed = discord.Embed(
        title="🤝 Ton code de parrainage",
        description=(
            f"Partage ce lien a tes amis pour les inviter sur TradePilot :\n\n"
            f"**{lien}**\n\n"
            "Quand quelqu'un achete via ton lien :\n"
            "→ Il beneficie du code **LAUNCH10** (-45%)\n"
            "→ Tu recois **1 mois Signal VIP GRATUIT**\n\n"
            "Pas de limite — plus tu parraines, plus tu gagnes ! 🚀"
        ),
        color=0xF5B830
    )
    embed.set_footer(text="TradePilot — Programme Parrainage")
...
# ════════════════════════════════════════════════════════
# ── STATS LIVE — EMBED INTERACTIF ───────────────────────
# ════════════════════════════════════════════════════════

PROFILS = {
    "micro":    {"nom": "Micro",    "min": "300€",  "lots": 0.01, "color": 0x7289da, "mult": 0.10},
    "starter":  {"nom": "Starter",  "min": "500€",  "lots": 0.02, "color": 0x23a55a, "mult": 0.20},
    "standard": {"nom": "Standard", "min": "1000€", "lots": 0.05, "color": 0xF5B830, "mult": 0.50},
    "pro":      {"nom": "Pro",      "min": "2000€", "lots": 0.10, "color": 0xe67e22, "mult": 1.00},
    "expert":   {"nom": "Expert",   "min": "5000€", "lots": 0.25, "color": 0xed4245, "mult": 2.50},
}

def build_stats_embed(profil_key="pro"):
    stats = _mt5_stats
    p = PROFILS[profil_key]
    mult = p["mult"]
    balance  = float(stats.get("balance", 0))
    pnl_jour = float(stats.get("pnl_jour", 0)) * mult
    pnl_sem  = float(stats.get("pnl_semaine", 0)) * mult
    pnl_mois = float(stats.get("pnl_mois", 0)) * mult
    winrate  = stats.get("winrate_semaine", 0)
    updated  = stats.get("updated_at", "—")
    trades   = stats.get("trades", [])
    currency = stats.get("currency", "€")
    has_data = balance > 0 or len(trades) > 0

    embed = discord.Embed(
        title="Résultats en direct — MT5",
        description=f"Profil : **{p['nom']}** · min {p['min']} · {p['lots']} lots",
        color=p["color"]
    )
    embed.set_author(name="TradePilot — Compte Live #3028785")

    if has_data:
        sj = "+" if pnl_jour >= 0 else ""
        ss = "+" if pnl_sem  >= 0 else ""
        sm = "+" if pnl_mois >= 0 else ""
        embed.add_field(name="Balance",      value=f"`{balance:.2f}{currency}`",         inline=True)
        embed.add_field(name="P&L Jour",     value=f"`{sj}{pnl_jour:.2f}{currency}`",    inline=True)
        embed.add_field(name="Winrate",      value=f"`{winrate}%`",                       inline=True)
        embed.add_field(name="P&L Semaine",  value=f"`{ss}{pnl_sem:.2f}{currency}`",     inline=True)
        embed.add_field(name="P&L Mois",     value=f"`{sm}{pnl_mois:.2f}{currency}`",    inline=True)
        embed.add_field(name="Trades ouverts", value=f"`{len(trades)}`",                 inline=True)
        if trades:
            lines = []
            for t in trades[:5]:
                pt = float(t.get("profit", 0)) * mult
                st = "+" if pt >= 0 else ""
                lines.append(f"**{t.get('symbol','—')}** `{t.get('type','—')}` {p['lots']} lots → `{st}{pt:.2f}{currency}`")
            embed.add_field(name="Positions ouvertes", value="\n".join(lines), inline=False)
    else:
        embed.add_field(name="En attente", value="Données MT5 non disponibles — VPS doit etre actif.", inline=False)

    embed.set_footer(text=f"TradePilot · Mis a jour : {updated} · Actualisation auto toutes les 30 min")
    return embed
...
class ProfilView(View):
    ...
    async def update_embed(self, interaction: discord.Interaction, profil_key: str):
        embed = build_stats_embed(profil_key)
        await interaction.response.edit_message(embed=embed, view=self)

    @discord.ui.button(label="Micro",    style=discord.ButtonStyle.secondary, custom_id="profil_micro")
    async def micro(self, interaction, button): await self.update_embed(interaction, "micro")

    @discord.ui.button(label="Starter",  style=discord.ButtonStyle.success,   custom_id="profil_starter")
    async def starter(self, interaction, button): await self.update_embed(interaction, "starter")

    @discord.ui.button(label="Standard", style=discord.ButtonStyle.primary,   custom_id="profil_standard")
    async def standard(self, interaction, button): await self.update_embed(interaction, "standard")

    @discord.ui.button(label="Pro",      style=discord.ButtonStyle.danger,    custom_id="profil_pro")
    async def pro(self, interaction, button): await self.update_embed(interaction, "pro")

    @discord.ui.button(label="Expert",   style=discord.ButtonStyle.danger,    custom_id="profil_expert")
    async def expert(self, interaction, button): await self.update_embed(interaction, "expert")


_stats_message = None

@tasks.loop(minutes=30)
async def stats_live_update():
    global _stats_message
    print(f"Stats task déclenchée — {len(bot.guilds)} serveurs")
    guild = None
    for g in bot.guilds:
        if g.id == GUILD_ID:
            guild = g
    ...
    if not guild:
        ...
        return
    channel = guild.get_channel(RESULTATS_CHANNEL_ID)
    if not channel:
        print(f"❌ Canal {RESULTATS_CHANNEL_ID} introuvable")
        return
    print(f"✅ Canal trouvé : {channel.name}")
    embed = build_stats_embed("pro")
    view  = ProfilView()
    if _stats_message:
        try:
            await _stats_message.edit(embed=embed, view=view)
            print("Stats embed mis a jour")
            return
        except Exception:
            _stats_message = None
    async for msg in channel.history(limit=20):
        if msg.author == bot.user and msg.embeds:
            if "MT5" in (msg.embeds[0].title or ""):
                _stats_message = msg
                await _stats_message.edit(embed=embed, view=view)
                print("Stats embed retrouve et mis a jour")
                return
    _stats_message = await channel.send(embed=embed, view=view)
    print("Stats embed poste pour la premiere fois")

@stats_live_update.before_loop
async def before_stats():
    await bot.wait_until_ready()
    await asyncio.sleep(10)  # Laisser le cache Discord se charger
    print("✅ Stats task prête à démarrer")
