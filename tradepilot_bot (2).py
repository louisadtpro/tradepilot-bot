...
class CloseTicketButton(View):
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: Button):
        await interaction.response.send_message("🔒 Ticket fermé dans 5 secondes — sauvegarde en cours...")

        # Sauvegarder les logs dans #logs-bot
        guild = interaction.guild
        logs_ch = guild.get_channel(1522505084318122155)
        if logs_ch:
            messages = []
            async for msg in interaction.channel.history(limit=100, oldest_first=True):
                ...
                if msg.content:
                    ...
                    messages.append(f"[{timestamp}] {msg.author.display_name}: [Embed] {msg.embeds[0].title or ''}")
            ...
            embed_log.add_field(name="Date", value=f"<t:{int(__import__('time').time())}:F>", inline=True)
...
async def create_ticket(interaction: discord.Interaction, ticket_type: str, label: str):
    ...
    # Notif admin dans #ventes-dashboard avec mention
    ...
    if ventes:
        ...
        await ventes.send(content="<@&1518731166277046372>", embed=embed_v)
...
async def on_member_join(member):
    ...
    # Message de bienvenue dans le salon arrivants
    membres_ch = member.guild.get_channel(1522177979709526056)
    if membres_ch:
        embed_v = discord.Embed(
            title=f"👋 Bienvenue sur TradePilot !",
            description=(
                f"**{member.display_name}** vient de rejoindre le serveur !\n\n"
                f"Tu es le **{member.guild.member_count}ème membre** 🎉\n\n"
                "→ Lis **#règles-et-infos** pour découvrir nos offres\n"
                "→ Pose tes questions dans **#général-trading**"
            ),
            color=0xF5B830
        )
        embed_v.set_thumbnail(url=member.display_avatar.url)
        embed_v.set_footer(text="TradePilot — Automatisation MT4 & MT5")
        await membres_ch.send(embed=embed_v)
